...
function addToCounts(counts, products, usePhysical = false) {
  for (const product of products) {
    ...
    counts[product.productName] =
      (counts[product.productName] || 0) + qty;
  }
}
...
async function processPDFs(filePaths) {
  ...
  for (const filePath of filePaths) {
    ...
    for (const pageText of pages) {
      ...
      const buyerNickname = getBuyerNickname(
        pageText,
        buyerId
      );

      const slipQtyTotal = Number(
        pageText.match(/Qty Total:\s*(\d+)/)?.[1] || 0
      );
      ...
      const parsedPdfQtyTotal = products.reduce(
        (sum, product) => sum + product.pdfQty,
        0
      );
      ...
      const quantityMatches =
        parsedPdfQtyTotal === slipQtyTotal;
  ...
  for (const [
    groupName,
    groupItemsRaw
  ] of Object.entries(groupedOrders)) {
    const groupItems = sortByNickname(
      groupItemsRaw
    );
    ...
    for (const item of groupItems) {
      ...
      for (const pageIndex of [
        order.labelPage,
        order.packingSlipPage
      ]) {
        if (
          pageIndex >= 0 &&
          pageIndex < inputPdf.getPageCount()
        ) {
          const [copiedPage] =
  ...
  for (const fileReport of verification.files) {
    verification.expectedOutputPages +=
      fileReport.expectedPagesUsed;

    for (
      let pageIndex = 0;
      pageIndex < fileReport.inputPageCount;
      pageIndex++
    ) {
      const pageKey =
        `${fileReport.fileName}::${pageIndex}`;
  ...
  const summary = {
    ...
    verificationPassed:
      verification.passed,
    inputPagesMatchOutput:
      verification.inputPagesMatchOutput,
    detectedOrdersMatchPages:
      verification.detectedOrdersMatchPages,
